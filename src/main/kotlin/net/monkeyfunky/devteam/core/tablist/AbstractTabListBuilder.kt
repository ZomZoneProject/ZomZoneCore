...
abstract class AbstractTabListBuilder : ITabListBuilder {
     ...
     */
    override fun getProfiles(player: Player): Array<TabListEntry> {
        ...
        while (profiles.size < 80) {
            ...
            if(profiles.size == 60) {
                ...
                profiles.add(TabListEntry())
                profiles.add(TabListEntry("${ ChatColor.DARK_AQUA }弓術: ${ status.getStatus(StatusTypes.BOW) }pt (+${ status.getStatus(StatusTypes.BOW).toDouble() / 10 })").setHead(HeadColor.DARK_AQUA))
                profiles.add(TabListEntry("${ ChatColor.YELLOW }剣術: ${ status.getStatus(StatusTypes.SWORD) }pt (+${ status.getStatus(StatusTypes.SWORD).toDouble() / 10 })").setHead(HeadColor.YELLOW))
                profiles.add(TabListEntry("${ ChatColor.RED }防御: ${ status.getStatus(StatusTypes.SHIELD) }pt (+${ status.getStatus(StatusTypes.SHIELD).toDouble() / 5 }%)").setHead(HeadColor.RED))
                profiles.add(TabListEntry("${ ChatColor.BLUE }体力: ${ status.getStatus(StatusTypes.HEALTH) }pt (+${ status.getStatus(StatusTypes.HEALTH).toDouble() / 10 })").setHead(HeadColor.BLUE))
                profiles.add(TabListEntry("${ ChatColor.DARK_PURPLE }銃葬: ${ status.getStatus(StatusTypes.GUN) }pt (+${ status.getStatus(StatusTypes.GUN).toDouble() / 5 })").setHead(HeadColor.DARK_PURPLE))
                profiles.add(TabListEntry())
